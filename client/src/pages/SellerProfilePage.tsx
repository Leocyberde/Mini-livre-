/**
 * SellerProfilePage - Página de perfil do lojista
...
export default function SellerProfilePage({ onBack }: { onBack?: () => void }) {
  ...
  // Pre-fill from seller profile, falling back to auth user registration data
  const [form, setForm] = useState({
    ...
    storePhone: sellerProfile.storePhone || user?.whatsapp || '',
    storeEmail: sellerProfile.storeEmail || user?.email || '',
  });

  const getInitialAddress = (): AddressFormType => {
    ...
    // Fall back to user's registration address
    if (user?.address?.cep) {
      return {
  ...
  const [address, setAddress] = useState<AddressFormType>(getInitialAddress);
